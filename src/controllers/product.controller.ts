...
import { addProductToDB, getProductById, getProductFromDB, updateProductById, deleteProductById } from "../services/product.service"; // Mengimpor fungsi untuk mendapatkan produk dari database MongoDB
...
export const updateProduct = async (req: Request, res: Response) => {
    // Mengambil parameter 'id' dari request body
    const {
        params: { id },
    } = req;
    ...
    try {
        const result = await updateProductById(id, value) // Mengupdate produk berdasarkan ID
        ...
        if (result) {
            ...
            logger.error("ERR: product - update product"); // Logging error
            res.status(404).send({
                status: false, // Status error
                statusCode: 404, // Kode status 404 (Not Found)
                message: "Product not found", // Pesan error
        ...
        logger.error("ERR: product - update product", error); // Logging error
        res.status(422).send({
            status: false, // Status validasi gagal
            statusCode: 422, // Kode status 422
            message: error// Pesan error dari validasi
...
export const deleteProduct = async (req: Request, res: Response) => {
    ...
    try {
        const result = await deleteProductById(id) // Menghapus produk berdasarkan ID
        if (result) {
            // Jika berhasil menghapus produk, kirim respon sukses
            logger.info("Success delete product"); // Logging sukses
            res.status(200).send({
                status: true, // Status sukses
                statusCode: 200, // Kode status 201 (OK)
                message: "Success delete product", // Pesan sukses
            })
        } else {
            // Jika produk tidak ditemukan, kirim respon error
            logger.error("ERR: product - delete product"); // Logging error
            res.status(404).send({
                status: false, // Status error
                statusCode: 404, // Kode status 404 (Not Found)
                message: "Product not found", // Pesan error
        ...
        // Jika terjadi kesalahan, kirim respon error
        logger.error("ERR: product - delete product", error); // Logging error
        res.status(422).send({
            status: false, // Status validasi gagal
            statusCode: 422, // Kode status 422
            message: error// Pesan error dari validasi
